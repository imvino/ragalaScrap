...
async function gen(data, webid,extDate=false) {
    let rdate = async () => {
        ...
        let delCount = 25;
        let ids = await database.sql("SELECT `rid` FROM `" + data + "` where `rdate` is null")
        ...
        async function run(id) {
            try {
                ...
                page.on('response', async (response) => {
                    if (response.url() === gotoUrl && response.status()) {
                        if (response.status() === 404) {
                            ...
                            await database.sql("UPDATE `" + data + "` SET `rdate` = '2222-02-17 00:00:00' WHERE `rid` = " + id)
                            await page.close();
                        }
                    }
                })
                page.on('load', async () => {
                    ...
                    await database.sql("UPDATE `" + data + "` SET `rdate` = '" + format.formatDate(time) + "' WHERE `rid` = " + id)
                    await page.close();
                })
...
gen('articles_interviews', 1709,true)
