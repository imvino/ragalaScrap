...
async function newUrlGen(db) {
    ...
    if (db == 'movies_function' || db == 'movies_photos' || db == 'movies_poster') {
        ...
        let r1 = await databaseCms("SELECT  `title`,`rid`,`permaLink` " + newtbl + " FROM `" + db + "` WHERE `permaLink` IS  NULL order by `rid` desc");
        if (r1.length == 0) console.log(db + ' over')
        r1.map((v, i) => {
            let rid = v.rid
    ...
    if (db == 'movies_reviews' || db == 'articles_editorial' || db == 'articles_interviews') {
        let r1 = await databaseCms("SELECT  `rid`,`permaLink`,`title`  FROM `" + db + "` WHERE `nurl` IS NULL and working='200' order by `rid` desc");
        if (r1.length == 0) console.log(db + ' over')
        r1.map((v, i) => {
            let rid = v.rid
            let gallery = v.permaLink ? urlSlug(v.permaLink) : urlSlug(v.title)
    ...
    if (db == 'articles_press_releases' || db == 'articles_news') {
        let r1 = await databaseCms("SELECT  `rid`,`permaLink`,`heading`  FROM `" + db + "` WHERE `nurl` IS NULL and working='200' order by `rid` desc");
        if (r1.length == 0) console.log(db + ' over')
        r1.map((v, i) => {
            let rid = v.rid
