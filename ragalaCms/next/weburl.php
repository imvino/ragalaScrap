...
$tables = ['articles_editorial' => 'articles', 'articles_interviews' => 'celebrity-interview', 'articles_press_releases' => 'press-release', 'local_events_events' => 'localevents', 'local_events_location' => 'localevents', 'movies_function' => 'functions', 'movies_names_title' => 'movies/info', 'movies_photos' => 'movies', 'movies_poster' => '', 'movies_reviews' => 'movies/review', 'starzone_filmpersonal' => 'stars/profile', 'starzone_photos' => '', 'articles_news' => 'tollywood-news-'];
//
foreach ($tables as $table => $key) {
    ...
    if ($table == 'local_events_events') {
        $title = 'eventLocationName';
    } elseif ($table == 'local_events_location') {
        $title = 'locationName';
    } elseif ($table == 'movies_names_title') {
        $title = 'movieName';
    } elseif ($table === 'movies_poster') {
        $title = 'imageName';
    } elseif ($table === 'starzone_filmpersonal') {
        $title = 'name';
    } elseif ($table === 'starzone_photos') {
        $title = 'title,gender';
    } elseif ($table === 'articles_press_releases') {
        $title = 'heading';
    ...
    $permaLink = $title != 'locationName' ? ',permaLink' : '';
    echo "SELECT $title,rid $permaLink FROM `" . $table . "` where web_url is NULL and working=200  ";
    ...
    while ($r = mysqli_fetch_assoc($sql)) {
        ...
        if ($title == 'imageName') {
            ...
            $url = '/' . $key . '/c/' . $r['rid'] . '/' . txtcleaner($r[$title]) . '.aspx';
        } else {
            $permalink = $r['permaLink'] ? $r['permaLink'] : $r[$title];
